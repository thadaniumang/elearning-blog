...
const qualifications = Array.from(document.querySelectorAll('input[name="qualification"]'));
const gender = Array.from(document.querySelectorAll('input[name="gender"]'));
const rating = Array.from(document.querySelectorAll('input[name="rate"]'));
const postSubmitText = document.querySelector('.submission-text');
...
const regExp = {
    ...
    stream: [/^(?!\s*$).+/, "Stream must be non-empty"],
...
const submitFormValidations = inputs => {
    ...
    checkRadio(qualifications, 'Please Select Your Qualifications');
    checkRadio(gender, 'Please Specify Your Gender');
    checkRadio(rating, 'Please Provide Ratings');
    inputs.forEach(input => {
...
form.addEventListener('reset', e => {
    postSubmitText.style.display = 'block'
    window.scrollTo(0, 0);
})